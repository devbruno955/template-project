...
function upload_image($input, $image=null, $type="team", $scale=false,$marcadagua=false) {
	...
	if ($z && strpos($z["type"], "image")===0 && $z["error"]==0) {
		...
		if ($type=="user") {
			...
			//Para adicionar marca d'água basta preencher as '' com o caminho do arquivo
		    ...
		    // $img->carrega(WWW_ROOT."/media/".$image)->redimensiona( $tampopw, $tampoph) ->marca( $INI['system']['wwwprefix'].'', 'baixo', 'centro' )->grava($npath);
			 
			$npath = preg_replace('#(\d+)\.(\w+)$#', "\\1_popular_mini.\\2", $path); 
			$img->carrega(WWW_ROOT."/media/".$image)->redimensiona( 47,28 )->grava($npath);
...
function getbannerslideshowhome(){ 
		...
		if($dh){
			...
			foreach ($itens as $file) {
				...
				$imagens.="<li><a href='#cubeJelly'><img src='$ROOTPATH/media/superbackground/".trim($file)."' class='fade' /></a></li>";
			 }  
		}  
...
function databra($var){
	$var  = explode("-",$var);
	$var = $var[0]."/".$var[1]."/".$var[2];
	return $var;
}
function datastr($var){
	$var  = explode("/",$var);
	$var = $var[0]."-".$var[1]."-".$var[2];
	return $var;
}
...
function UrlAnuncio($id) {
	...
	/* São buscados informações acerca do anúncio, estado, cidades. */
...
function LinkBreadcrumb($id) {
	...
	if(empty($tipos)) {
		...
		$url = $ROOTPATH . "/finalidade-todos/" . $tipos . "/varios-quartos/varias-vagas/todos-bairros/todas-cidades/todos-estados/#";
	}
	
	/* Cidade a qual o anúncio pertence. */
	$cidade = strtolower(tratamento_string(retira_acentos($cidade["nome"])));
	
	if(empty($cidade)) {
		$cidade = "todas-cidades";
	}
	else {
		$url .= $ROOTPATH . "/finalidade-todos/" . $tipos . "/varios-quartos/varias-vagas/todos-bairros/" . $cidade . "/todos-estados/#";
	}
	
	/* Estado a qual o anúncio pertence. */
	...
	if(empty($estado)) {		
		...
		$url .= $ROOTPATH . "/finalidade-todos/" . $tipos . "/varios-quartos/varias-vagas/todos-bairros/" . $cidade . "/" . $estado ."/";
	}
	return $url;
}
function verreserva($inicioperiodo,$fimperiodo,$idanuncio,$hoscrianca,$hosadulto){
     
	 if(empty($inicioperiodo)){
		$inicioperiodo =  date("d/m/Y");
		$fimperiodo 	= date('d/m/Y', strtotime("+1 days"));
	 }
	
	$_SESSION[fimperiodo] 	 	= datastr($fimperiodo);
	$_SESSION[inicioperiodo] 	= datastr($inicioperiodo);
	$_SESSION[idanuncio] 		= datastr($idanuncio);
	...
	Util::log("verreserva($inicioperiodo,$fimperiodo,$idanuncio,$hoscrianca,$hosadulto)" );  
	$semdata = false;
	$qtdehospede = $hoscrianca + $hosadulto;
	
	$inicioperiodo = datadb($inicioperiodo);
	$fimperiodo = datadb($fimperiodo);
	
	 $sql = "SELECT id FROM reservas WHERE statuspagamento = 'aprovado' and idanuncio=$idanuncio and '$inicioperiodo' BETWEEN inicioestadia AND fimestadia";
	 $rs = mysql_query($sql);
	// Util::log("sql 1".$sql ); 
	 if(mysql_num_rows($rs) > 0){
		$semdata = true;
		 ...
		 $sql = "SELECT id FROM reservas WHERE statuspagamento = 'aprovado' and  idanuncio=$idanuncio and '$fimperiodo' BETWEEN inicioestadia AND fimestadia";
		 $rs = mysql_query($sql);
		 //Util::log("sql 2".$sql ); 
		 if(mysql_num_rows($rs) > 0){
			$semdata = true;
	...
	$sql = "SELECT id FROM reservas WHERE statuspagamento = 'aprovado' and idanuncio=$idanuncio and inicioestadia BETWEEN '$inicioperiodo' AND '$fimperiodo'";
    $rs = mysql_query($sql); 
	 if(mysql_num_rows($rs) > 0){
		$semdata = true;
	 }
	...
	if($semdata){ //o nome da class deve ser diasindisponivel para que o javascript funcione e nao deixe prosseguir
		$txt  = "<div class='diasindisponivel'>O período selecionado está</div>";
		$txt  .= "<div class='diasindisponiveldestaque'>INDISPONÍVEL</div>";
		echo  utf8_encode($txt); 
		return;
	}	 
     $sql 	= "SELECT DATEDIFF('$fimperiodo',  '$inicioperiodo' ) as dias FROM reservas limit 1";
	 $rs 	= mysql_query($sql);
	 $row 	= mysql_fetch_assoc($rs);
	 $dias 	= $row[dias]; 
	 if($dias==""){
			$txt  = "<div class='diasindisponivel'> Código 00569. Entre em contato conosco </div>";
			$txt  .= "<div class='diasindisponiveldestaque'></div>";
			echo  utf8_encode($txt); 
			return;
	 }
	 
	 if($dias<0){ //o nome da class deve ser diasindisponivel para que o javascript funcione e nao deixe prosseguir
		$txt  = "<div class='diasindisponivel'>Informe uma data válida</div>";
		$txt  .= "<div class='diasindisponiveldestaque'>DATA INVÁLIDA</div>";
		echo  utf8_encode($txt); 
		return;
	 }
	 
	 $anuncio = Table::Fetch('team', $idanuncio);
	 if(!empty($anuncio[minimo_dias_alugar])){
		if( (int)$dias < (int)$anuncio[minimo_dias_alugar] ){ 
			$txt  = "<div class='diasindisponivel'>Selecione no mínimo ".$anuncio[minimo_dias_alugar]." dias - ($dias) </div>";
			$txt  .= "<div class='diasindisponiveldestaque'>PERÍODO INVÁLIDO</div>";
			echo  utf8_encode($txt); 
			return; 
		}
	 }
	 
	 if( $qtdehospede > $anuncio[maximo_hospede] ){ 
			$txt  = "<div class='diasindisponivel'>O imóvel comporta até ".$anuncio[maximo_hospede]." hóspedes</div>";
			$txt  .= "<div class='diasindisponiveldestaque'>MUITOS HÓSPEDES</div>";
			echo  utf8_encode($txt); 
			return;  
	 } 
	 
	$price 				= number_format($anuncio[team_price] ,0, '.', ''); 
	$valordias 			= $anuncio[team_price] * $dias; 
	 
	if($anuncio[tipo] == "onlineparcial"){
	
		$valorreservaonline = number_format($valordias,2, ',', '.'); 
		$restantelocal 		= number_format($valordias,2, ',', '.'); 
	
		$taxa_parcial_reservar 	= $anuncio[taxa_parcial_reservar]; 
		$valorreservaonline 	= ( $valordias *  $taxa_parcial_reservar) / 100 ;
		$restantelocal 			= ($anuncio[team_price] * $dias) - $valorreservaonline;
		  
	}
	  
	$_SESSION[diarias] 			= $dias;
	$_SESSION[valordiaria] 		= $price;
	$_SESSION[valortotal] 		=  number_format($valordias ,0, '.', '');   
	
	$txt  = "<div class='datasdispo'>Suas datas estão <span>disponíveis</span></div>";
	$txt .= "<div class='linhainfopreco'>";
	$txt .= "<div class='precodiaria'>Preço de $dias diária(s)</div>";
	$txt .= "<div class='totaldiarias'>R$ ".number_format($valordias ,2, ',', '.') ." </div>";
	$txt .= "</div>"; 
	if($anuncio[tipo] == "onlineintegral"){
		$_SESSION[valorreserva]  	= number_format($valordias,2, ',', '.');
		$_SESSION[valorrestante]  	= "";
		 
		$txt .= "<div class='titreserveapenas'>Total a pagar</div>";
		$txt .= "<div class='valorreserveapenas'>R$ ".number_format($valordias,2, ',', '.')."</div>"; 
	
	}	
	else if($anuncio[tipo] == "onlineparcial"){
	
		$_SESSION[valorreserva]  	= number_format($valorreservaonline ,0, '.', '');  
		$_SESSION[valorrestante]  	= number_format($restantelocal ,0, '.', ''); 
		 
		$txt .= "<div class='titreserveapenas'>Reserve agora por apenas</div>";
		$txt .= "<div class='valorreserveapenas'>R$ ".number_format($valorreservaonline,2, ',', '.')."</div>";
		$txt .= "<div class='pagamentolocal'>Pagamento de R$ ". number_format($restantelocal,2, ',', '.')." no local</div>";
	
	}
	else if($anuncio[tipo] == "pagamentooffline"){
		$_SESSION[valorreserva]  	= "";
		$_SESSION[valorrestante]  	= "";
		
		$txt .= "<div class='titreserveapenas'>Pague somente no local</div>";
		$txt .= "<div class='valorreserveapenas'>R$ ".number_format($valordias,2, ',', '.')."</div>"; 
...
function valores_reserva($inicioperiodo,$fimperiodo,$idanuncio){
	Util::log("verreserva()" );  
	$semdata = false;
	$qtdehospede = $hoscrianca + $hosadulto;
	...
	if($semdata){ //o nome da class deve ser diasindisponivel para que o javascript funcione e nao deixe prosseguir
	 ...
	 if(!empty($anuncio[minimo_dias_alugar])){
		if( (int)$dias < (int)$anuncio[minimo_dias_alugar] ){ 
			$txt  = "<div class='diasindisponivel'>Selecione no mínimo ".$anuncio[minimo_dias_alugar]." dias </div>";
			$txt  .= "<div class='diasindisponiveldestaque'>PERÍODO INVÁLIDO</div>";
			echo  utf8_encode($txt); 
			return; 
		}
	...
	$price 				= number_format($anuncio[team_price] ,0, '.', ''); 
	$valordias 			= (int)$price * (int)$dias;
	$valordias 			= number_format($valordias,2, ',', '.'); 
	$valorreservaonline = number_format($valordias,2, ',', '.'); 
	$restantelocal 		= number_format($valordias,2, ',', '.'); 
	 
	if($anuncio[tipo] == "onlineparcial"){
		$taxa_parcial_reservar 	= $anuncio[taxa_parcial_reservar]; 
		$valorreservaonline 	= ( $anuncio[team_price] * $dias *  $taxa_parcial_reservar) / 100 ;
		$restantelocal 			= ($anuncio[team_price] * $dias) - $valorreservaonline;
		
		/* FORMATAÇÃO*/
		$valorreservaonline 	= number_format($valorreservaonline,2, ',', '.');
		$restantelocal 			= number_format($restantelocal,2, ',', '.');
		
	}
	
	//$txt  = "<div class='boxdispo'>";
	$txt  = "<div class='datasdispo'>Suas datas estão <span>disponíveis</span></div>";
	$txt .= "<div class='linhainfopreco'>";
	$txt .= "<div class='precodiaria'>Preço de $dias diária(s)</div>";
	$txt .= "<div class='totaldiarias'>R$ $valordias </div>";
	$txt .= "</div>";
	$txt .= "<div class='titreserveapenas'>Reserve por apenas</div>";
	$txt .= "<div class='valorreserveapenas'>R$ $valorreservaonline</div>";
	$txt .= "<div class='pagamentolocal'>Pagamento de R$ $restantelocal no local</div>";
	//$txt .= "</div>";
	 echo  utf8_encode($txt); 
	 return;
	  
}
function get_valor_parcial($idanuncio){
		
		$anuncio = Table::Fetch('team', $idanuncio);
  
		$taxa_parcial_reservar 	= $anuncio[taxa_parcial_reservar]; 
		$valorreservaonline 	= ( $anuncio[team_price] * $taxa_parcial_reservar) / 100 ;  
		/* FORMATAÇÃO*/
		$valorreservaonline 	= number_format($valorreservaonline,2, ',', '.'); 
		return $valorreservaonline;
	 
}
function envia_email_reserva($id){
	global $INI,$ROOTPATH; 
	$reserva 	= Table::Fetch('reservas', $id);
	$user 		= Table::Fetch('user', $reserva['idusuario']);
	...
	$body = file_get_contents($ROOTPATH."/templates/dados_reserva.php", false, $request);
	if(Util::postemailCliente($body,ASSUNTO_DADOS_RESERVA,$user['email'])){
		Util::log($id. " - envia_email_reserva - Email para o cliente enviado com sucesso ".$user['email']);  
		$enviaemail = true;
	 }
	else{
		Util::log($id. " - envia_email_reserva - Erro no envio do email para ".$user['email']);  
	}
	
	$bodyadmin="----------  CÓPIA DE EMAIL ENVIADA PARA O ADMINISTRADOR ----------<BR>";
	$mensagem =  $bodyadmin.$mensagem; 
	enviar( $INI['mail']['from'] ,ASSUNTO_NOVA_RESERVA_ADMIN,$mensagem);  
	$mensagem="";
	isset($mensagem);
	return $enviaemail;
}
...
/*
function getdatasagendadas(){

	$sql = "select team_id, dataevento from `reservas` where dataevento is not null and dataevento <> ''  and dataevento <> '1969-12-31'  and dataevento <> '0000-00-00'   ";
	$rs = mysql_query($sql);
	while($row = mysql_fetch_assoc($rs)){ 
		 $dataevento = $row[dataevento];
		 $team = Table::Fetch('team',$row[team_id]);
		 $limiteeventopordia = $team[limiteeventopordia];
		 
		 $sql2 = "select count(id) as qtde from `order` where dataevento='$dataevento'  ";
		 $rs2 = mysql_query($sql2);
		 $row2 = mysql_fetch_object($rs2);
		 $qtde_atual =  $row2 ->qtde;
		 
		 if($qtde_atual >=  $limiteeventopordia){
			$datas[] = databra($dataevento) ;
		 }
	} 
	  return $datas;
}
...
function date_range($dates, $first, $last, $step = '+1 day', $output_format = 'j/n/Y' ) {
...
function getReservas_anuncio($id){

	$sql = "select * from `reservas` where idanuncio = $id and statuspagamento = 'aprovado' order by id";
	$rs = mysql_query($sql);
	while($row = mysql_fetch_assoc($rs)){ 
		$dates = date_range($dates, $row['inicioestadia'], $row['fimestadia']);
	}
	 return $dates;
}

function trata_datas($str){
	$array =  explode(",",$str);
	foreach($array as $key) {
	    $key = datadb(trim($key)); 
		$current = strtotime($key);
		$key = date('j/n/Y', $current ); 
		//echo "--".$key;
		$newarray[] =  $key;
	}
	return $newarray;
