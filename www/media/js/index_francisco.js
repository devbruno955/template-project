(function () {
    ...
    jQuery.fn = jQuery.prototype = {
            ...
            if (typeof selector === "string") {
                ...
                if (match && (match[1] || !context)) {
                    if (match[1]) selector = jQuery.clean([match[1]], context);
                        ...
                        if (elem && elem.id != match[3]) return $().find(selector);
                        var ret = $(elem || []);
                        ret.context = document;
                        ret.selector = selector;
                        return ret
                    }
                } else return $(context).find(selector)
            } else if (jQuery.isFunction(selector)) return $(document).ready(selector);
        ...
        closest: function (selector) {
            var pos = jQuery.expr.match.POS.test(selector) ? $(selector) : null,
                closer = 0;
        ...
        add: function (selector) {
            return this.pushStack(jQuery.unique(jQuery.merge(this.get(), typeof selector === "string" ? $(selector) : jQuery.makeArray(selector))))
        },
    ...
    jQuery.each({
        ...
        jQuery.fn[name] = function (selector) {
            ...
            for (var i = 0, l = insert.length; i < l; i++) {
                ...
                jQuery.fn[original].apply($(insert[i]), elems);
    ...
    jQuery.fn.extend({
        ...
        die: function (type, fn) {
            $(document).unbind(liveConvert(type, this.selector), fn ? {
        ...
        load: function (url, params, callback) {
            ...
            jQuery.ajax({
                ...
                complete: function (res, status) {
                    if (status == "success" || status == "notmodified") self.html(selector ? $("<div/>").append(res.responseText.replace(/<script(.|\s)*?\/script>/g, "")).find(selector) : res.responseText);
        ...
        animate: function (prop, speed, easing, callback) {
            ...
            return this[optall.queue === false ? "each" : "queue"](function () {
                var opt = jQuery.extend({}, optall),
                    p, hidden = this.nodeType == 1 && $(this).is(":hidden"),
                    self = this;
                for (p in prop) {
    ...
    jQuery.extend({
        speed: function (speed, easing, fn) {
            ...
            opt.complete = function () {
                if (opt.queue !== false) $(this).dequeue();
        ...
        step: function (gotoEnd) {
            ...
            if (gotoEnd || t >= this.options.duration + this.startTime) {
                ...
                if (done) {
                    ...
                    if (this.options.hide) $(this.elem).hide();
    ...
    jQuery.each(['Left', 'Top'], function (i, name) {
        ...
        jQuery.fn[method] = function (val) {
            ...
            return val !== undefined ? this.each(function () {
                this == window || this == document ? window.scrollTo(!i ? val : $(window).scrollLeft(), i ? val : $(window).scrollTop()) : this[method] = val
...
var validator = {
    ...
    checkRepeat: function (value) {
        ...
        return value == $('input[name="' + to + '"]').eq(0).val()
    },
    ...
    checkCompare: function (value) {
        ...
        return $('#' + compareid).attr('value') == value
    },
    Exec: function (value) {
            ...
            success: function (data) {
                ...
                if (data.error != 0) {
                    ...
                    if (msgid.length > 0) {
                        ...
                        $("<span class='" + validator.errorTip + "'></span>").html(errmsg).insertAfter(element)
                    }
                    return false
                    ...
                    if (msgid.length > 0) {
                        ...
                        $("<span class='" + validator.validTip + "'></span>").insertAfter(element)
                    }
                    return true
                }
            }
...
validator.showErr = function (element, errindex) {
    ...
    if (msgid.length > 0) {
        ...
        $("<span class='" + this['errorTip'] + "'></span>").html(errmsg).insertAfter(element)
    }
    return false
};
...
jQuery.fn.validateForm = function (no, form) {
    ...
    elements.each(function (i) {
        if (false == validator.check($(this), true)) {
            var m = $(this).parent('*').find('.' + validator.errorTip).html();
            if (m) {
                emsg += (emsg == '') ? m : '\n' + m
            }
            isValid = false;
...
window.x_init_hook_validator = function () {
    ...
    $('a.needlogin').click(X.misc.needlogin)
};
window.x_init_hook_dealimage = function () {
    var teamside = $('#team_partner_side_0').size() > 0;
    var m = teamside ? 650 : 410;
    if (teamside) {
        $('#team_main_side').css('width', '410px');
        $('.side #side-Business img').each(function () {
    ...
    $('#deal-stuff img').each(function () {
        X.misc.scaleimage(this, m)
    });
    $('div.sbox-content img').each(function () {
        X.misc.scaleimage(this, 195)
    })
};
window.x_init_hook_myaccount = function () {
    var ma = $('#myaccount');
    var mm = $('#myaccount-menu');
    ma.hover(function () {
        mm.show();
...
window.x_init_hook_click = function () {
    $("div:not(#guides-city-change)").click(function () {
    ...
    $('#deal-share-im').click(function () {
        return !$('#deal-share-im-c').toggle()
    });
    $('a.ajaxlink').click(function () {
        if ($(this).attr('no') == 'yes') return false;
	...
	$('a.ajaxlink_edit').click(function () {
        if ($(this).attr('no') == 'yes') return false;
        var link = $(this).attr('href');
        var ask = $(this).attr('ask');
        if (link.indexOf('/delete') > 0 && !confirm('确定删除本条记录吗？')) {
    ...
    $('#cardcode-verify-id').click(X.misc.cardcode);
    $('#consult-add-form input[name="commit"]').click(function () {
        $('#consult-add-form').ajaxSubmit({
            'success': function () {
    ...
    $('#consult-add-more').click(X.team.consultation_again);
    $('#express-zone-div input').click(function () {
        var v = $(this).attr('value');
        if (v == 'express') {
            $('#express-zone-express').css('display', 'block');
            $('#express-zone-pickup').css('display', 'none');
            $('#express-zone-coupon').css('display', 'none')
        } else if (v == 'pickup') {
            $('#express-zone-pickup').css('display', 'block');
            $('#express-zone-express').css('display', 'none');
            $('#express-zone-coupon').css('display', 'none')
        } else if (v == 'coupon') {
            $('#express-zone-coupon').css('display', 'block');
            $('#express-zone-pickup').css('display', 'none');
            $('#express-zone-express').css('display', 'none')
        }
    });
    $('#mail-zone-div input').click(function () {
        var v = $(this).attr('value');
        if (v == 'smtp') {
            $('#mail-zone-smtp').css('display', 'block')
        } else {
            $('#mail-zone-smtp').css('display', 'none')
        }
    });
...
window.x_init_hook_imagerotate = function () {
    ...
    $('#team_images').everyTime(3000, 'imagerotate', imagesRun);
    $('#team_images li,#img_list a').hover(function () {
        $('#team_images').stopTime('imagerotate')
    }, function () {
        $('#team_images').everyTime(3000, 'imagerotate', imagesRun)
    });
    ...
    var imagesPlay = function (next) {
        $('#team_images li').eq(imgListCurr).css({
            'opacity': '0.5'
        }).animate({
            'left': '-440px',
            'opacity': '1'
...
window.x_init_hook_clock = function () {
    var a = parseInt($('div.deal-timeleft').attr('diff'));
    if (!a > 0) return;
    ...
    var e = function () {
        ...
        if (ls > 0) {
            ...
            $("ul#counter").stopTime('counter');
            $('ul#counter').html('end');
            window.location.reload()
        }
    };
    $("ul#counter").everyTime(996, 'counter', e)
};
window.x_init_hook_team = function () {
    $('#deal-buy-quantity-input').bind("keyup", function () {
        var n = parseInt($(this).attr('value'), 10);
        var per = parseInt($('#deal-per-number').attr('value'), 10);
        if (n > per && per > 0) {
            n = per
        }
        n = isNaN(n) ? '' : n;
        $(this).attr('value', n);
        var p = parseFloat($('#deal-buy-price').html(), 10);
        p = isNaN(p) ? 0 : p;
        n = isNaN(n) ? 0 : n;
        var t = n * p;
        $('#deal-buy-total').html(t.toFixed(2));
        X.team.dealbuy_farefree(n);
        X.team.dealbuy_totalprice()
    })
};
window.x_init_hook_order = function () {
    $('form[id="order-pay-form"]').bind('submit', function () {
        X.get(WEB_ROOT + '/ajax/order.php?action=dialog&id=' + $(this).attr('sid'))
    })
};
X.misc = {};
...
X.misc.scaleimage = function (o, mw) {
    var w = $(o).width();
    if (w > mw) $(o).css('width', mw + 'px')
};
X.misc.inputblur = function () {
    var v = $(this).attr('value');
    var t = $(this).attr('xtip');
    if (v == t || !v) {
        $(this).attr('value', t);
        $(this).css('color', '#999')
    }
...
X.misc.noticenext = function (tid, nid,qtdeenviohora,tipo) {
    $('#dialog_subscribe_count_id').html(nid);
...
X.misc.noticenextsms = function (tid, nid) {
    $('#dialog_smssubscribe_count_id').html(nid);
...
X.misc.cardcode = function () {
    var oid = $('#cardcode-order-id').attr('value');
    var cid = $('#cardcode-card-id').attr('value');
    if (oid && cid) return !X.get(WEB_ROOT + '/ajax/order.php?action=cardcode&id=' + oid + '&cid=' + cid)
};
X.misc.smscount = function () {
    var l = $('#sms-content-id').val().length;
...
X.team.dealbuy_farefree = function (n) {
    var ff = parseInt($('#deal-buy-quantity-input').attr('ff'));
    if (ff > 0 && ff <= n) {
        $('#deal-express-total').html('0')
    } else {
        var fa = parseInt($('#deal-express-price').attr('v'));
        $('#deal-express-total').html(fa)
    }
};
X.team.dealbuy_totalprice = function () {
    var n = parseFloat($('#deal-buy-total').html(), 10);
    n = isNaN(n) ? 0 : n;
    var p = parseFloat($('#deal-express-total').html(), 10);
    p = isNaN(p) ? 0 : p;
    var t = n + p;
    $('#deal-buy-total-t').html(t.toFixed(2))
};
X.team.changetype = function (type) {
    var display = (type == 'goods') ? 'none' : 'block';
    if (type == 'goods') $('#team-create-per-number').val('0');
    if (type != 'seconds') {
        $('#team-create-begin-time').val($('#team-create-begin-time').attr('xd'));
        $('#team-create-end-time').val($('#team-create-end-time').attr('xd'));
        $('#team-create-begin-time').attr('maxLength', 10);
        $('#team-create-end-time').attr('maxLength', 10)
    } else {
        $('#team-create-begin-time').val($('#team-create-begin-time').attr('xd') + ' ' + $('#team-create-begin-time').attr('xt'));
        $('#team-create-end-time').val($('#team-create-end-time').attr('xd') + ' ' + $('#team-create-end-time').attr('xt'));
        $('#team-create-begin-time').attr('maxLength', 19);
        $('#team-create-end-time').attr('maxLength', 19)
    }
    $('#field_city').css('display', display);
    $('#field_num').css('display', display);
    $('#field_notice').css('display', display);
    $('#field_card').css('display', display);
    $('#field_userreview').css('display', display);
    $('#field_systemreview').css('display', display);
    $('#field_limit').css('display', display)
};
X.coupon = {};
X.coupon.dialogquery = function () {
    var id = $('#coupon-dialog-input-id').attr('value');
    if (id) return !X.get(WEB_ROOT + '/ajax/coupon.php?action=query&id=' + encodeURIComponent(id))
};
X.coupon.dialogconsume = function () {
    var id = $('#coupon-dialog-input-id').attr('value');
    var secret = $('#coupon-dialog-input-secret').attr('value');
    if (id && secret) {
        var ask = $('#coupon-dialog-consume').attr('ask');
...
X.coupon.dialoginputkeyup = function (o) {
    $(o).attr('value', $(o).attr('value').toUpperCase())
};
X.manage = {};
...
X.manage.usermoney = function () {
    ...
    var money = parseFloat($('#user-dialog-input-id').attr('value'));
    var uid = $('#user-dialog-input-id').attr('uid');
    var ask = $('#user-dialog-input-id').attr('ask');
...
X.manage.orderexpress = function () {
    var eid = parseInt($('#order-dialog-select-id').val());
    var nid = $('#order-dialog-input-id').attr('value');
    var oid = $('#dialog-order-id').attr('oid');
...
X.manage.orderrefund = function () {
    var rid = $('#order-dialog-refund-id').val();
    var oid = $('#dialog-order-id').attr('oid');
...
function getEndereco() {
		...
		if($.trim($("#imob_cep").val()) != ""){
			...
			*/
			$.getScript("http://cep.republicavirtual.com.br/web_cep.php?formato=javascript&cep="+$("#imob_cep").val(), function(){
				...
				if(resultadoCEP["resultado"]){
					...
					$("#city_id").val(CidadeUsuario);
					
					$("#imob_estado").val(unescape(resultadoCEP["uf"])); 
					$('#select_imob_estado').text($('#imob_estado').find('option').filter(':selected').text());
					// $("#imob_estado").trigger("change"); 
					...
					//$('#city_id').text(CidadeUsuario );  
					 //$('#select_city_id').text($('#city_id').find('option').filter(':selected').text());
					
					
					//$("#imob_bairro").val(unescape(resultadoCEP["bairro"]));
					$('#imob_bairro').val( unescape(resultadoCEP["bairro"]) );
					$('#imob_rua').val( unescape( unescape(resultadoCEP["tipo_logradouro"])+"  "+unescape(resultadoCEP["logradouro"])));
				}else{
