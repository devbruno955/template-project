...
elseif ( 'deletetipo' == $action) {
	$team = Table::Fetch('team', $id, 'imob_tipo');
	if ($team) {
		Session::Set('notice', "Existem anúncios cadastrados com esse tipo. Exclua esses anúncios primeiro");
		echo "<script language='JavaScript'> 
			window.location='/vipmin/category/tipoimoveis.php'; 
		</script>";
	}else{
		Table::Delete('tipoimoveis', $id);
		Session::Set('notice', "Tipo {$id} excluído com sucesso");
		echo "<script language='JavaScript'> 
			window.location='/vipmin/category/tipoimoveis.php'; 
		</script>";
	}
...
elseif ( 'deletecidade' == $action) {
		
		Table::Delete('cidades', $id);
		Session::Set('notice', "Cidade {$id} excluída com sucesso");
		echo "<script language='JavaScript'> 
			window.location='/vipmin/category/indexcidades.php'; 
		</script>";
...
elseif ( 'duplicar' == $action) {
	...
	$sql = "INSERT INTO team (   `user_id`,  `title`, `summary`, `city_id`, `group_id`, `partner_id`, `system`, `team_price`,     `begin_time`, `end_time`,`imob_adicionais`,`imob_vagas`,`imob_quartos`,`imob_bairro`,`imob_tipo`,`imob_finalidade`,`imob_estado`,`anunciogratis`,`status`,`pago`,`mostrarpreco`,`create_time`) 
	SELECT  `user_id`,   `title`, `summary`, `city_id`, `group_id`, `partner_id`, `system`, `team_price`,    `begin_time`, `end_time`,`imob_adicionais`,`imob_vagas`,`imob_quartos`,`imob_bairro`,`imob_tipo`,`imob_finalidade`,`imob_estado`,`anunciogratis`,`status`,`pago`,`mostrarpreco`,`create_time` FROM team
	WHERE id = ".$id ;
	$rs = mysql_query($sql);
	
	if($rs){
		Session::Set('notice', "Anúncio {$order['id']} duplicado com sucesso. Não esqueça de fazer o upload das fotos. Atenção: Só duplique ofertas do mesmo tipo. Por exemplo, não duplique ofertas do tipo promocional para ofertas do tipo normal.");
	}
	else{
		Session::Set('notice', "Erro ao duplicar - ".mysql_error());
	}
	json(null, 'refresh');
}
