...
if ( !$edicao ) { // cadastro
	...
	if( !is_post()){
		include template('manage_partner_adminanunciante_edit');
	}
	else{
		...
		$table->image1 = upload_image('upload_image1', null, 'faixada', true);
		...
		$flag = $table->insert(array(
			...
			'password', 'address', 'open', 'display', 'entrega_telefone',
	...
	if(!is_post()){
		...
		$table->group_id = abs(intval($table->group_id));
		$table->username 	= $table->contact;
		$table->contact 	= strip_tags($_POST["contact"]);
		$table->city_id = abs(intval($table->city_id));
		$table->open = (strtoupper($table->open)=='Y') ? 'Y' : 'N';
		$table->display = (strtoupper($table->display)=='Y') ? 'Y' : 'N';
		$table->image = upload_image('upload_image', null, 'parceiro', true);  
		...
		if($table->password ){
			...
			$up_array = array(
					...
					'location', 'other', 'homepage', 'contact', 'mobile', 'phone',
					'address', 'group_id', 'open', 'city_id', 'display', 'entrega_telefone',
