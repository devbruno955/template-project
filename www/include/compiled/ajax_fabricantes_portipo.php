...
<? if($tipo=="carro"){?>
	...
	<select  name="car_fabricante" id="car_fabricante" onchange="$('#car_fabricante_id').text($('#car_fabricante').find('option').filter(':selected').text())"> 
		<option value=""> </option>
		<?php 
...
<? if($tipo=="caminhao"){?>
	...
	<select  name="car_fabricante" id="car_fabricante" onchange="$('#car_fabricante_id').text($('#car_fabricante').find('option').filter(':selected').text())"> 
		<option value=""> </option>
		<?php 
			$sql = "SELECT id, nome FROM fabricante where tipo = 'Caminhão' order by nome";
			$rs = mysql_query($sql);
			while($l = mysql_fetch_assoc($rs)){
				if (isset($team['car_fabricante']) && $team['car_fabricante'] == $l['id']) {
					echo "<option value='$l[id]' selected='selected'>".displaySubStringWithStrip($l[nome],40)."</option>";
