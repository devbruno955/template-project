...
function buscaTituloAnuncio($team){ 
	...
	$titulo = $estado. ' - ' .$cidade . ', ' . utf8_encode(getBairro($team['imob_bairro'])) . ', ' . $Tipo . " (" . utf8_encode($finalidade) . ")";
	return $titulo; 
}
...
function customSubString($txt, $size){

	if(strlen($txt) > $size){
		$aux = substr(strip_tags($txt),0,$size); 
		$val = strrpos($aux," "); // Aqui ele acha o último espaço em branco da nossa string limitada em 80 caracteres
		$stringlimitada = substr(strip_tags($txt),0,$val)." ..."; // Aqui ele limita ela no valor que nós achamos na linha anterior e adiciona os três pontinhos
