...
function upload_image($input, $image=null, $type="team", $scale=false,$marcadagua=false) {
	...
	if ($z && strpos($z["type"], "image")===0 && $z["error"]==0) {
		...
		if ($type=="user") {
			...
			Image::Convert($path, $npath, 220, null, Image::MODE_CUT);
...
function getEnderecoClienteEntrega($dados){ 
	...
	$cidade_estado_cep = ", ".ucfirst(utf8_decode($dados['entrega_cidade'])). " - ".strtoupper(utf8_decode($dados['entrega_estado'])). " - ".$dados['entrega_cep'];
	
	if($dados['entrega_bairro']!="" || $dados['entrega_bairro']!=" "){
		$endereco .=  ", ".ucfirst(utf8_decode($dados['entrega_bairro'])).$cidade_estado_cep;
	}else{
		$endereco .=  $cidade_estado_cep;
	}
	//echo "Bairro: ".$dados['entrega_bairro'];
	echo $endereco ;
...
function buscaTituloAnuncio($team){ 
	...
	$tipoFinalidade = ' , ' . $Tipo . ", (" . utf8_encode($finalidade) . ")";
	
	if(utf8_encode(getBairro($team['imob_bairro']))!="" || utf8_encode(getBairro($team['imob_bairro']))!=" "){
		$titulo = $estado_cidade . ", ". $team['imob_bairro'] . $tipoFinalidade; 
	}else{
		$titulo = $estado_cidade . $tipoFinalidade;
	}		
...
function buscaTituloAnuncioMobile($team){ 
	$titulo = buscaTituloAnuncio($team);
...
function UrlAnuncio($id) {
	...
	$url =  $tipos . "-" . $finalidade  . "-" . $estado . "-" . $cidades;
	$url = URLify::filter($url);
	return $ROOTPATH."/imovel/".$url. "/" . $id . "/";
}
...
function urlAtual(){
 ...
 function formataCPF($num){
	...
	if($tipo == "cpf"){
		$new = $num[0].$num[1].$num[2].".".$num[3].$num[4].$num[5].".".$num[6].$num[7].$num[8]."-".$num[9].$num[10];
		return $new;
	}
	if($tipo == "cnpj"){
		$new = $num[0].$num[1].".".$num[2].$num[3].$num[4].".".$num[5].$num[6].$num[7]."/".$num[8].$num[9].$num[10].$num[11]."-".$num[12].$num[13];
		return $new;
	}

}

function isfree($idpartnerplano){
	$partner_plano = Table::Fetch("partner_planos", $idpartnerplano);

	return $partner_plano['status'];
}

function countPhotos($team){
	$c = 0;
	if($team['img0']){
		$c++;
	}
